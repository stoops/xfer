...
void *xfer(void *argv)
{
	...
	else
	{
		...
		{
			cons->flag = "syns"; stop = 1;
		}
	}

	if (stop == 0) { cons->stat = 1; }
	else { cons->stat = 9; }
...
void *mgmt(void *argv)
{
	...
	while (1)
	{
		...
		for (int x = 0; x < NUMB; ++x)
		{
			if (cons[x].stat == 2)
			{
				if ((secs - cons[x].last) >= news)
				{
					cons[x].flag = "news"; cons[x].stat = -2;
				}
			}
			if (cons[x].stat == 1)
			{
				...
				{
					cons[x].flag = "time"; cons[x].stat = -2;
				}
			}
			if (cons[x].stat < 0)
			{
				timo = gtim(args->timo, cons[x].dprt);
				printf("%s INFO fins [%s:%s] * [%s:%d] [%s:%d:%d] [%s]\n", date(), cons[x].srcs, cons[x].port, cons[x].dsts, cons[x].dprt, args->prot, cons[x].stat, timo, cons[x].flag); fflush(stdout);
				fins(&cons[x].xfer);
				fins(&cons[x].conn);
				fins(&cons[x].remo);
				if (cons[x].stat == -1)
