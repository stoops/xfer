...
def answ(qhed, qbdy, qdns, data, flag):
	...
	try:
		...
		for item in resl:
			try:
				rdat = dns.rdata.from_text(dns.rdataclass.IN, item["type"], item["data"])
				rrse = dns.rrset.from_rdata(qbdy.name.decode(), item["TTL"], rdat)
				rdns.answer.append(rrse)
...
def send(head, body, qdns, outp, flag, serv, addr):
	try:
		if (not outp):
			logs("WARN", "outp")
		else:
			repl = answ(head, body, qdns, outp, flag)
	...
	if (head and body):
		...
		if (name in cach.keys()):
			try:
				...
				send(head, body, qdns, outp, flag, serv, addr)
				if ((secs - last) >= (5 * 60)):
					flag = True
					refr = True
			except Exception as e:
