...
for i in os.environ.get("TIMO", "").split(";"):
	try:
		j = i.split(":")
		p = j[0].split(",")
		t = int(j[1])
...
def xfer(args):
	...
	(prot, srcl, path, addr, conn, quel, maps) = (args["prot"], args["srcl"], args["path"], args["addr"], args["conn"], args["quel"], args["maps"])
...
def mgmt(args):
	...
	(clis, maps, quel) = (args["clis"], args["maps"], args["quel"])
	last = 0 ; nums = 0
...
def main():
	...
	logs("INFO",TIMO,"args",prot,host,srcl,path)
	...
	args = {"clis":clis, "maps":maps, "quel":quel}
		...
		for fdes in rfds:
			...
			if (prot != "tcp"):
				...
				if (not addr in maps.keys()):
					conn = ssoc
					dest = comd(path, addr, prot)
					temp = sock(prot, addr, srcl, dest, conn)
					maps[addr] = temp
					temp.send("sock", data)
				else:
					objc = maps[addr]
			...
			if (not objc):
				args = {"prot":prot, "srcl":srcl, "path":path, "addr":addr, "conn":conn, "quel":quel, "maps":maps, "this":None}
